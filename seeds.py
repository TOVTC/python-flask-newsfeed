...
# anytime we want to perform a CRUD operation using SQLAlchemy, we need to establish a temporary session connection
db = Session()

# insert users
# db.add_all() does not change the database, it just preps the SQL queries
# you will need to call db.commit() to save changes and then db.close() the connection when you are finished
db.add_all([
...
# don't forget that Post references User so the users need to be seeded first
db.add_all([
